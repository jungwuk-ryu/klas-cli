...
- `auth login`은 자격 증명을 확인한 뒤 재사용 가능한 로컬 세션을 만듭니다.
- `auth status`는 로컬 세션과 환경변수 상태를 확인해 현재 인증 가능 여부를 보여줍니다.
- 인증이 필요한 일반 명령은 먼저 이 로컬 세션을 확인합니다.
- 로컬 세션이 없으면 `KLAS_ID`, `KLAS_PASSWORD`를 사용합니다.
- 둘 다 없으면 대화형 터미널에서만 직접 입력을 받습니다.
- `auth logout`은 로컬 세션만 지우며, 셸의 환경변수는 그대로 둡니다.
